...
<head>
    ...
    <title>تایید حذف - تنظیم نهایی موقعیت</title>
    <style>
        @font-face {
...
<div class="screen">
    <div class="delete-modal">
        <div class="modal-text">
            آیا از حذف کاربر {{ $user->fullname }}<br>مطمئن هستید ؟
        </div>

        <form action="{{ route('manager.users.destroy',['user' => $user]) }}" method="post" class="modal-actions">
            @csrf
            @method('DELETE')
            <button class="btn">بله</button>


            <a href="{{ route('manager.users.index') }}" class="btn">خیر</a>
        </form>
    </div>
</div>
