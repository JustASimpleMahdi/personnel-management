...
<div class="screen">
    <form action="{{ route('manager.announcements.update',compact('announcement')) }}" method="post" class="main-card">
        @csrf
        @method('PUT')
        <!-- فیلد عنوان -->
        <div class="input-group">
            <label>عنوان:</label>
            <input name="title" value="{{ old('title',$announcement->title) }}" type="text" class="input-field">
        </div>

        <!-- فیلد شرح اعلانیه -->
        <div class="input-group">
            <label>شرح اعلانیه:</label>
            <textarea name="text" class="input-field">{{ old('text',$announcement->text) }}</textarea>
        </div>
